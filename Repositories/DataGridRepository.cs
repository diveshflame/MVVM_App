    ...
    public class DataGridRepository : RepositoryBase, IAdminBooking
    {
        ...
        public List<DataGridItem> ViewHistory()
        {
            ...
            using (NpgsqlConnection conn = GetConnection())
            {
                conn.Open();
                string view = "SELECT NAME,DOCTOR_NAME,STARTTIME, ENDTIME, CONSULTANT_DESC FROM USERDETAILS INNER JOIN BOOKING_TABLE ON USERDETAILS.USERID = BOOKING_TABLE.USERID INNER JOIN DOCTOR_TABLE ON " +
                    "BOOKING_TABLE.DOCTOR_ID = DOCTOR_TABLE.DOCTOR_ID INNER JOIN CONSULTANT_TYPE ON DOCTOR_TABLE.CONSULTANT_ID = CONSULTANT_TYPE.CONSULTANT_ID AND DELETED_TIMESTAMP IS NULL AND DATE(STARTTIME) < CURRENT_DATE ORDER BY STARTTIME ";
                NpgsqlCommand cmd = new NpgsqlCommand(view, conn);
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
        ...
        public List<DataGridItem> Search(string searchContent)
        {
            ...
            using (NpgsqlConnection conn = GetConnection())
            {
                conn.Open();
                string view = "SELECT NAME,DOCTOR_NAME,STARTTIME, ENDTIME, CONSULTANT_DESC FROM USERDETAILS INNER JOIN BOOKING_TABLE ON USERDETAILS.USERID = BOOKING_TABLE.USERID INNER JOIN DOCTOR_TABLE ON " +
                    "BOOKING_TABLE.DOCTOR_ID = DOCTOR_TABLE.DOCTOR_ID INNER JOIN CONSULTANT_TYPE ON DOCTOR_TABLE.CONSULTANT_ID = CONSULTANT_TYPE.CONSULTANT_ID WHERE (DOCTOR_NAME=@NAME OR NAME=@NAME OR CONSULTANT_DESC=@NAME ) AND DELETED_TIMESTAMP IS NULL AND DATE(STARTTIME)>=CURRENT_DATE ORDER BY STARTTIME";
                NpgsqlCommand cmd = new NpgsqlCommand(view, conn);
                cmd.Parameters.AddWithValue("@NAME", searchContent);
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
